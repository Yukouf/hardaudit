...
| Réseau | 12 | CIS 3.x | Écoutes wildcard, validation anti-spoofing (`rp_filter`), refus des routes IPv4 imposées par la source, exceptions d'interface désactivant la politique ou le chiffrement IPsec, blocage optionnel des annonces ARP gratuites sur réseaux statiques, création de voisins depuis des ARP non sollicités (`arp_accept`), réponses aux requêtes ARP pour d'autres hôtes par `proxy_arp`/`proxy_arp_pvlan` (interception de trafic), en-têtes de routage IPv6 type 2, réception SRv6 sans HMAC obligatoire, rejet des annonces de voisin IPv6 non sollicitées sur Wi-Fi, détection des adresses IPv6 dupliquées (DAD), apprentissage par les routeurs de voisins IPv6 absents du cache (`accept_untracked_na`), journalisation des sources impossibles, routage inhabituel de `127/8`, refus des paquets usurpant une adresse IPv4 locale, redirects ICMP IPv4/IPv6 et portée réelle de leur confiance (`shared_media` peut neutraliser `secure_redirects`), relais des broadcasts dirigés IPv4, routeurs acceptant encore les annonces IPv6 (`accept_ra=2`), RA dont la source appartient déjà à l'hôte, refus des PMTU de protocole forgeables pendant le forwarding, limitation des boucles d'ACK sur segments TCP invalides, absence de quota global observable sur les Challenge ACK, protection TCP TIME-WAIT contre les RST, randomisation des horodatages TCP, refus des requêtes ICMP broadcast/multicast et masque de limitation ICMP non vide |
...
| Kernel | 14 | CIS 1.6 / ANSSI R14 | ASLR, entropie mmap, décalage aléatoire de la pile kernel à chaque syscall, protections contre les vulnérabilités CPU désactivées par `mitigations=off`, isolation des caches slab, randomisation de l'allocateur de pages, mitigation des split locks x86, détection NMI des blocages CPU durs et détection des tâches bloquées en état D avec budget d'alertes, ptrace, perf_events et limiteur CPU de l'échantillonnage, syncookies (modes `1` et test permanent `2` reconnus), pile LSM et présence d'une politique MAC, accès non-root aux interfaces kernel `debugfs`/`tracefs`, BPF non privilégié et durcissement du JIT BPF (`bpf_jit_harden` et publicité des adresses JIT dans kallsyms), core dumps privilégiés, collecteurs pipe sans borne et chemin du helper root, chemins des helpers privilégiés `kernel.modprobe` et `kernel.hotplug`, répétition illimitée des oops et warnings kernel, segments de mémoire partagée SysV orphelins, limites des files de messages SysV (`msgmni`×`msgmnb` sans quota global), io_uring globalement ouvert et sans médiation AppArmor sur les kernels compatibles, userfaultfd non restreint par sysctl **ou délégué via `/dev/userfaultfd`**, memfd exécutables par défaut, namespaces utilisateur sans médiation AppArmor, exceptions `unconfined` et anciennes ABI de politique, page mémoire nulle, autoload TTY et injection TIOCSTI historique, verrou kexec et limites de chargement des images normales/de crash, modules chargeables sans signature obligatoire/Lockdown, interprètes `binfmt_misc` héritant des privilèges du binaire, masquage des pointeurs kernel (modes renforcés acceptés), protections hardlink/symlink/FIFO/fichiers de `/tmp`, écriture illimitée dans le ring buffer kernel via `kernel.printk_devkmsg`, limite de cartes mémoire `vm.max_map_count` déclarée dans `/etc/sysctl.d` mais inactive (dérive politique/état effectif) |
...
# Proxy ARP : la valeur effective est le OU entre all et l'interface ; 1 = répond pour d'autres hôtes
grep -H . /proc/sys/net/ipv4/conf/{all,default,*}/{proxy_arp,proxy_arp_pvlan} 2>/dev/null
ip route show
ip neigh show
...
# SRv6 entrant : activé par interface ; seul require_hmac=1 refuse les paquets sans HMAC
grep -H . /proc/sys/net/ipv6/conf/{default,*/}/{seg6_enabled,seg6_require_hmac} 2>/dev/null
ip -6 route show

# Sur Wi-Fi, le kernel recommande de jeter les annonces de voisin IPv6 non sollicitées
for i in /sys/class/net/*/wireless; do n=${i%/wireless}; n=${n##*/}; grep -H . /proc/sys/net/ipv6/conf/$n/drop_unsolicited_na; done

# Sur un routeur, 1/2 permet à une NA de créer un voisin absent ; drop_unsolicited_na=1 prime
grep -H . /proc/sys/net/ipv6/conf/{default,*/}{accept_untracked_na,drop_unsolicited_na,forwarding} 2>/dev/null
ip -6 neigh show

# Détection d'adresse IPv6 dupliquée : max(all, interface)=0 la désactive
grep -H . /proc/sys/net/ipv6/conf/{all,default,*}/accept_dad 2>/dev/null
ip -6 addr show
...
# Un routeur ne doit normalement pas faire confiance aux PMTU de protocole, forgeables
sysctl net.ipv4.ip_forward net.ipv4.ip_forward_use_pmtu

# Protection RFC 1337 : 1 empêche un RST de supprimer prématurément l'état TCP TIME-WAIT
sysctl net.ipv4.tcp_rfc1337

# Délai minimal entre les ACK dupliqués répondant aux segments TCP invalides ; 0 retire la limite
sysctl net.ipv4.tcp_invalid_ratelimit

# Quota partagé de Challenge ACK : le kernel déconseille de l'activer ; INT_MAX = illimité
...
# Files de messages SysV : msgmni files × msgmnb octets, sans quota global
...
# Publicité des adresses JIT dans /proc/kallsyms (0 = non exportées, défaut)
# bpf_jit_harden actif désactive cet export ; kptr_restrict borne qui peut le lire
sysctl net.core.bpf_jit_kallsyms net.core.bpf_jit_enable net.core.bpf_jit_harden \
  kernel.kptr_restrict
grep -Ec 'bpf_prog_[0-9a-f]+' /proc/kallsyms
...
# debugfs/tracefs : l'upstream réserve la racine debugfs à root par défaut
findmnt -t debugfs,tracefs -o TARGET,FSTYPE,OPTIONS
stat -c '%a %A %U %G %n' /sys/kernel/debug /sys/kernel/tracing
...
> **Faux positif debugfs/tracefs :** un groupe d'observabilité peut recevoir volontairement un accès aux traces kernel. Cette délégation expose toutefois des interfaces dont le contenu et les commandes varient selon les modules chargés ; HardAudit signale donc tout propriétaire non-root ou bit d'accès groupe/autres sur la racine montée. Préférer un collecteur privilégié qui exporte seulement les métriques nécessaires plutôt qu'un accès brut.
...
> **Faux positif adresses JIT dans kallsyms :** `net.core.bpf_jit_kallsyms=1` est utile au traçage (bpftool, perf) de programmes JIT sans libérer totalement leur adresse, et le kernel ne l'expose qu'aux utilisateurs privilégiés. La gravité réelle dépend de qui peut lire `/proc/kallsyms` : `kernel.kptr_restrict=0` laisse tout compte local lire les adresses et supprime l'entropie KASLR de la zone JIT (cas sensible), tandis qu'`une valeur > 0` borne la lecture aux processus avec CAP_SYSLOG. HardAudit ne signale le cas que si le JIT est actif et le masquage `bpf_jit_harden` absent — ce dernier désactive l'export par conception, quelle que soit la valeur du sysctl. Passer à `0` si le traçage ne l'exige pas.
...
> **Faux positif tâches bloquées :** `hung_task_timeout_secs=0` désactive réellement la détection, mais un délai trop court peut produire du bruit sur un stockage lent ou pendant certaines opérations de maintenance. `hung_task_warnings=0` garde les contrôles actifs mais supprime tout nouvel avertissement ; `-1` autorise des alertes sans limite. HardAudit n'exige pas `hung_task_panic=1`, qui transformerait un blocage détecté en indisponibilité. Vérifier avec `sysctl kernel.hung_task_timeout_secs kernel.hung_task_warnings kernel.hung_task_panic`.
...
> **Faux positif files de messages SysV :** le défaut Linux (`msgmni=32000`, `msgmnb=16384`) suffit à réserver plusieurs centaines de Mo de mémoire noyau par un compte local, mais ces files sont rarement exploitées hors des applications SysV dédiées. HardAudit signale le défaut en INFO (aucune pénalité) et ne relève le finding en LOW que si `msgmni` ou `msgmnb` dépassent le défaut. Réduire les limites exige de vérifier les applications SysV existantes et les files réellement créées (`ipcs -q`).
...
> **Faux positif proxy ARP :** `proxy_arp` est un mécanisme légitime de bascule IP (keepalived/VRRP), de NAT et de certains routages vers des machines ne répondant pas elles-mêmes aux ARP. Le kernel l'active dès que `conf/{all,interface}/proxy_arp` vaut `1` (OU logique), donc une valeur `all=1` arme chaque interface. Un hôte qui proxy-répond aux requêtes ARP pour des adresses qu'il sait joindre devient un routeur de substitution utilisable pour l'interception du trafic ; vérifier avec `ip route show` quelle adresse est réellement jointe avant de conserver ce mode. `proxy_arp_pvlan` (RFC 3069) ne répond que sur la même interface pour une isolation de switch de type VLAN privé.
...
> **Faux positif SRv6 :** un routeur ou une plateforme réseau programmable peut être volontairement un endpoint Segment Routing. HardAudit ne signale que les interfaces où `seg6_enabled` est actif et où `seg6_require_hmac=1` n'impose pas l'authentification des paquets entrants ; le constat prouve une capacité locale, pas un détournement observé. Désactiver SRv6 si inutile, ou déployer les clés HMAC sur tous les pairs avant de rendre le HMAC obligatoire.

> **Faux positif annonces de voisin IPv6 en Wi-Fi :** `drop_unsolicited_na=1` convient à 802.11 pour éviter qu'un voisin injecte une association IPv6/MAC non sollicitée, mais peut casser un proxy NDP connu ou une bascule HA qui dépend de ces annonces. HardAudit cible uniquement les interfaces identifiées comme Wi-Fi par sysfs ; valider ces usages avant activation.

> **Faux positif DAD IPv6 :** désactiver Duplicate Address Detection peut être volontaire pour de l'anycast, une bascule HA ou certaines interfaces de conteneurs. HardAudit cible uniquement les interfaces non-loopback qui possèdent déjà une adresse IPv6 et calcule la valeur effective `max(all, interface)` ; le finding signale que les prochaines adresses ne seront pas vérifiées, pas qu'un doublon existe déjà.

> **Faux positif voisins IPv6 non suivis :** `accept_untracked_na=1` optimise volontairement le premier trafic retour d'un routeur RFC 9131 quand les hôtes émettent une NA avec `ndisc_notify`; le mode `2` limite en plus la source au sous-réseau local. HardAudit ne signale que les interfaces qui routent et où `drop_unsolicited_na=0`, car ce dernier réglage a priorité. Le finding décrit une capacité d'apprentissage, pas un empoisonnement observé.
...
> **Faux positif Challenge ACK :** une limite globale peut sembler réduire le trafic, mais son épuisement partagé devient observable depuis une autre connexion et fournit un canal auxiliaire. Le kernel recommande désormais `INT_MAX` et conserve des limites par socket ; HardAudit signale donc toute borne inférieure, sans prétendre qu'une attaque est en cours.
